...
class PainterContainer extends StatefulWidget {
  final int id;
  final String title;
  const PainterContainer({Key? key, required this.id, required this.title})
      : super(key: key);
...
class _PainterContainer extends State<PainterContainer> {
  ...
  void initState() {
    ...
    if (widget.title != "") _loadPoints(widget.id, widget.title);
  }
  ...
  Future<void> _savePoints(int id, String title) async {
    final List<Map<String, dynamic>> data =
        PointPainter.points[id].map((e) => e.toMap()).toList();
    ...
    if (snapshot.exists) {
      ...
      if (points != null) {
        setState(() {
          ...
          PointPainter.points[id]
              .addAll(points.map((e) => PaintData.fromMap(e)).toList());
        });
      }
    }
  }
